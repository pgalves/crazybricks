...
DualBuilder::DualBuilder(int width, int height):m_currentMap(NULL){
    m_width = width;
    m_height = height;
...
void DualBuilder::buildMap(ObjectFactory& factory){
    //left wall
...
void DualBuilder::buildBlock(ObjectFactory& factory, QWidget *game){
...
void DualBuilder::buildPlatform(ObjectFactory& factory){
    ...
    PBehaviour *topBehaviour = new DumbBehaviour(*factory.getEngine(), *(this->getMap()), topPlat->getId());
    ...
    PBehaviour *bottomBehaviour = new DumbBehaviour(*factory.getEngine(), *(this->getMap()), bottomPlat->getId());
    bottomPlat->setBehaviour(bottomBehaviour);
    m_currentMap->setBottomPlatform(bottomPlat->getId());
}

void DualBuilder::buildBall(ObjectFactory& factory){
