...
static const int pointsBonus = 400;
static const int lifesBonus = 1;
static const int bonusRadius = 12;

EasyFactory::EasyFactory(PhysicSystem& engine, Map& map) :
    ObjectFactory(engine, map){}

MapObject* EasyFactory::createBlock(float posX, float posY,int width, int height){
    Block* block = new Block(posX, posY, width, height, StateBlockCollided::Instance());
...
MapObject* EasyFactory::createWallBlock(float posX, float posY,int width, int height){
...
MapObject* EasyFactory::createBall(float posX, float posY, ballInit location){
    Ball* ball;
    if(location == TOPBALL) {
...
MapObject* EasyFactory::createPlatform(float posX, float posY,int width, int height, int limitXLeft, int limitXRight){
...
MapObject* EasyFactory::createBonus(float posX, float posY, bonusType type, int bonusToPlatformId, CrazyBricks* game){
//    Bonus* bonus = new FreezePlatBonusDecorator(new LifesBonusDecorator(new Bonus(posX, posY, bonusRadius, bonusToPlatformId, pointsBonus, game), lifesBonus), 5);
    Bonus* bonus = new Bonus(posX, posY, bonusRadius, bonusToPlatformId, pointsBonus, game);
...
PaintVisitor* EasyFactory::createPaintVisitor(QWidget *widget){
    return new SimplePaintVisitor(widget);
}

void EasyFactory::createBackground(QWidget *widget){
    QPalette palette;
    palette.setBrush(widget->backgroundRole(), QBrush(QPixmap(":/images/simplespace.png")));
